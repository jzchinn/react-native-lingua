...
3. **Disable every scout you did NOT enable** in (a)–(c) — this is now most of the troop — **except one already enabled before this run.** `scout-config-sync`'s response tells you each config's current `enabled` state; a config that was already `true` before you started tuning is someone's deliberate choice (built-in or custom), not something this setup gets to switch off. Restrict `scout-config-update` disables to configs that were **not** already enabled coming into this step. For a config you didn't pick that was already enabled, leave it alone and record it in the report as an existing enabled scout outside this run's ≤10 pick (it still counts toward the troop's shared ceiling — factor it in when sizing step 6b's room). Disable via `scout-config-update` with the config `id` and `{ enabled: false }` — **nothing else** — for the configs that do qualify. Don't touch `emit` (dry-run posture) or `run_interval_minutes`; the defaults are correct. A failed update is a follow-up, not an abort.
